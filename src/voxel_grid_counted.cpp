...
namespace vofod
{
  ...
  VoxelGridCounted::VoxelGridCounted(const float threshold)
  {
  ...
  void VoxelGridCounted::filterImpl(PointCloudOut& output)
  {
    ...
    for (const auto& [first_index, last_index] : first_and_last_indices_vector)
    {
      ...
      const auto first_it = std::next(std::begin(input_->points), first_index);
      const auto last_it = std::next(std::begin(input_->points), last_index);
      const auto count = std::count_if(first_it, last_it, [this](const auto pt) {return pt.intensity > threshold_;});
